...
public class MemberUIHandler {
    ...
    public MemberUIHandler(Scanner scanner, MemberDAO memberDAO, GymClassDAO gymClassDAO,
            MerchandiseDAO merchandiseDAO, UserAuth currentUser) {
    ...
    private void viewMembershipInfo() {
        ...
        for (Member member : allMembers) {
            if (member.getId() == currentUser.getUserId()) {
                ...
                System.out.println(UIHelper.BRIGHT_GREEN + "Membership Type: " + UIHelper.RESET + UIHelper.BRIGHT_YELLOW
                        + member.getMembershipType() + UIHelper.RESET);
                System.out.println(UIHelper.BRIGHT_GREEN + "Status: " + UIHelper.RESET
                        + (member.isActive() ? UIHelper.BRIGHT_GREEN + "ACTIVE" : UIHelper.BRIGHT_RED + "INACTIVE")
                        + UIHelper.RESET);
                return;
            }
        }
    ...
    private void memberViewAndEnrollClasses() {
        ...
        while (browsing) {
            ...
            for (GymClass gymClass : classes) {
                ...
                System.out.println(UIHelper.BRIGHT_CYAN + "[" + gymClass.getClassId() + "]" + UIHelper.RESET + " " +
                        gymClass.getClassName() + " | Schedule: " + gymClass.getClassSchedule() + " | " + availability
                        + UIHelper.RESET);
            }
    ...
    private void memberShopMerchandise() {
        ...
        while (shopping) {
            ...
            for (Merchandise item : items) {
                String inStock = item.getQuantityInStock() > 0 ? UIHelper.BRIGHT_GREEN + "IN STOCK"
                        : UIHelper.BRIGHT_RED + "OUT OF STOCK";
                System.out.println(UIHelper.BRIGHT_CYAN + "[" + item.getProductId() + "]" + UIHelper.RESET + " " +
                        item.getProductName() + " | $" + String.format("%.2f", item.getPrice()) + " | " +
                        inStock + UIHelper.RESET + " | " + item.getCategory());
            }
